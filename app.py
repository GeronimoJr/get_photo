...
# TODO: Refactor large functions (process_images_in_parallel) into smaller, testable units (item 7)
# TODO: Enhance session state management and user notifications, including state file cleanup (item 8)
# TODO: Implement streaming/chunked file reading for large CSV/XML to reduce memory usage (item 9)
# TODO: Unify language usage across all logs and UI messages (item 10)
...
class FTPManager:
    ...
    def get_instance(cls, settings):
        key = f"{settings['host']}:{settings['username']}:{settings['directory']}:{settings.get('use_tls')}:{settings.get('http_path','')}"
        with cls._lock:
    ...
    def _send_keepalive(self):
        ...
        try:
            ...
            logger.error(f"Błąd keepalive: {e}", exc_info=True)
            self.connected = False
            ...
            try:
                if self.ftp:
                    try:
                        ...
                        logger.warning(f"Błąd zamknięcia poprzedniego połączenia FTP: {e}", exc_info=True)
        ...
        with self.connection_semaphore:
            ...
            for attempt in range(max_retries):
                ...
                try:
                    ...
                    logger.error(f"Błąd wysyłania FTP (próba {attempt+1}): {e}", exc_info=True)
                    self.last_error = str(e)
                    self.error_count += 1
                    self.connected = False
                    if attempt < max_retries - 1:
    ...
    def _process_queue(self):
        ...
        while self.running:
            try:
                ...
                logger.error(f"Błąd FTPBatchManager: {e}", exc_info=True)
                time.sleep(0.5)
    def _upload_file(self, task_id, file_path):
        ...
        with self.semaphore:
            try:
                ...
                try:
                    upload_result = conn.upload_file(file_path)
                finally:
    ...
    try:
        ...
        for retry in range(3):
            try:
                ...
                if FTP_CONFIG['MIN_IMAGE_SIZE'] <= file_size <= FTP_CONFIG['MAX_IMAGE_SIZE']:
                    return {"path": file_path, "filename": filename, "original_url": url}, None
                else:
                    os.remove(file_path)
                    if retry < 2:
                        continue
                    if file_size <= FTP_CONFIG['MIN_IMAGE_SIZE']:
                        if log_list is not None:
...
def process_images_in_parallel(urls, temp_dir, ftp_settings, max_workers=None, debug_container=None, max_retries=3, progress_callback=None, session_id=None, file_info=None, processing_params=None):
        ...
        while not retry_queue.empty() or next_url_index < total_urls or ftp_tasks:
            ...
            if retry_queue.empty() and next_url_index >= total_urls and ftp_tasks:
                if progress_callback and total_urls > 0:
                    ...
                    progress = stats['total_uploaded'] / total_urls
                ...
                try:
                    ...
                    if progress_callback and total_urls > 0:
                        if images_since_update >= 10 or batch_processed == batch_size_total:
                            ...
                            progress_callback(progress, processed_count, stats['total_uploaded'], total_urls)
                            images_since_update = 0
                            if session_id and file_info and processing_params:
                                save_processing_state(session_id, urls, list(new_urls_map.keys()), new_urls_map, file_info, processing_params)
                except Exception as e:
    ...
    if progress_callback and total_urls > 0:
        stats = ftp_batch_manager.get_stats()
        progress_callback(1.0, processed_count, stats['total_uploaded'], total_urls)
...
def handle_errors(func):
    def wrapper(*args, **kwargs):
        try:
            ...
            logger.error(f"Błąd w funkcji {func.__name__}: {e}", exc_info=True)
            return None, f"Błąd: {str(e)}"
    return wrapper
...
def extract_image_urls_from_csv(csv_content, column_name, separator=","):
    # Parsowanie CSV w trybie chunk, by nie ładować całego pliku do pamięci
    try:
        df_iter = pd.read_csv(io.StringIO(csv_content), chunksize=1000)
    except Exception as e:
        return None, f"Błąd przy wczytywaniu CSV: {e}"
    urls = []
    for df in df_iter:
        ...
        for value in df[column_name]:
            if pd.notna(value):
                ...
                if separator in val_str:
                    urls.extend([url.strip() for url in val_str.split(separator) if url.strip()])
                else:
                    urls.append(val_str.strip())
    return urls, None
...
def save_processing_state(session_id, urls, processed_urls, new_urls_map, file_info, processing_params):
    ...
    with open(state_file, "w") as f:
        json.dump(state, f)
    logger.info(f"Saved processing state to {state_file}")
    return state_file
...
def resume_processing(state, temp_dir, ftp_settings, max_workers=5):
    ...
    if not remaining_urls:
        ...
        # Po zakończeniu usuwamy plik stanu sesji
        state_dir = os.path.join(os.path.expanduser("~"), ".xml_image_processor")
        state_file = os.path.join(state_dir, f"session_{session_id}.json")
        if os.path.exists(state_file):
            os.remove(state_file)
            logger.info(f"Usunięto plik stanu sesji: {state_file}")
        return True
    ...
    processed_offset = len(state.get("processed_urls", []))
    total_offset = 0
    progress_callback = make_progress_callback(start_time, progress_bar, status_text, processed_offset, total_offset)
...
def make_progress_callback(start_time, progress_bar, status_text, processed_offset=0, total_offset=0):
    """Returns a standardized progress callback."""
    def callback(progress, processed, uploaded, total):
        overall_processed = processed_offset + processed
        overall_total = total_offset + total
        percent = int(progress * 100)
        elapsed = time.time() - start_time
        speed = uploaded / elapsed if elapsed > 0 else 0
        remaining = (overall_total - overall_processed) / speed if speed > 0 else 0
        status_text.text(
            f"Przetwarzanie... {overall_processed}/{overall_total} ({percent}%) | "
            f"Przesłano na FTP: {uploaded} | Szybkość: {speed:.1f} obrazów/s | ~{int(remaining/60)}m {int(remaining%60)}s pozostało"
        )
        progress_bar.progress(overall_processed / overall_total)
    return callback
        ...
        if st.session_state.file_info and st.button("Pobierz zdjęcia i prześlij na FTP"):
            try:
                ...
                with tempfile.TemporaryDirectory() as tmpdirname:
                    ...
                    progress_callback = make_progress_callback(start_time, progress_bar, status_text)
                    new_urls_map, downloaded_images, failed_urls = process_images_in_parallel(
                        ...
                        processing_params=st.session_state.processing_params
                    )
                ...
                try:
                    processed = list(new_urls_map.keys())
                except:
                    processed = []
                save_processing_state(session_id, urls, processed, locals().get('new_urls_map', {}), st.session_state.file_info, st.session_state.processing_params)
