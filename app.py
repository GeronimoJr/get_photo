...
class FTPManager:
    ...
    def upload_file(self, fp, rn=None):
        ...
        try:
            ...
            url=hp+rn if hp else f"ftp://{self.s['host']}/{self.s['directory'].strip('/')}/{rn}"
...
def init_state():
    defaults={
        "file_info":None,
        "ftp_settings":dict(host="",port=21,username="",password="",directory="/",http_path=""),
        "processing_params":dict(xpath="",column_name="",new_node_name="",new_column_name="",separator=","),
        "output_bytes":None
    }
...
def save_state(sid, all_urls, processed, new_map, file_info, params, failed=None):
    ...
    state={
        ...
        "file_info":dict(name=file_info["name"],type=file_info["type"],encoding=file_info["encoding"]),
        "processing_params":params,
        "all_urls":all_urls,
        "processed_urls":processed,
        "failed_urls":failed,
        "remaining_urls":rem,
        "new_urls_map":new_map,
...
def list_sessions():
    d=os.path.join(os.path.expanduser("~"),".xml_image_processor"); out=[]
    if os.path.exists(d):
            ...
            if f.startswith("session_"):
                try:
                    ...
                    out.append(dict(
                        ...
                        file=stt["file_info"]["name"],
                        progress=f"{len(stt['processed_urls'])}/{stt['total_urls']}",
                        percentage=pct
                    ))
                except: pass
    ...
    try:
        ...
        for _ in range(3):
            try:
                ...
                if os.path.getsize(fp)>100: return dict(path=fp,filename=fn),None
...
def process_parallel(urls,td,ftp_s,max_workers,debug=None):
    ...
    def proc(u):
        ...
        if err: return dict(status="download_error",url=u,error=err)
        fm=FTPManager(ftp_s)
        if not fm.connect(): return dict(status="ftp_error",url=u,error="FTP connect")
        res=fm.upload_file(img["path"]); fm.close(); time.sleep(0.5)
        if res["success"]: return dict(status="success",url=u,ftp_url=res["url"],filename=res["filename"])
        return dict(status="upload_error",url=u,error=res.get("error",""))
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as ex:
        ...
        for f in concurrent.futures.as_completed(fut):
            ...
            if r["status"]=="success":
                nm[r["url"]]=r["ftp_url"]
                dl.append(dict(original_url=r["url"],ftp_url=r["ftp_url"],filename=r["filename"]))
                if debug: debug.success(f"Pobrano: {r['url']}")
            else:
                fail.append(dict(url=r["url"],error=r.get("error","")))
...
def extract_xml(c,xp,sep):
    try:
        ...
        if xp in ("//product/image","product/image"):
            ...
            return [u.replace('&amp;','&').strip() for u in m if u.strip()],None
...
def update_xml(c,xp,map_,nn,sep):
    try:
        ...
        for el in els:
            ...
            parent=pm[el]
            ftp_p=parent.find("ftp_images") or parent.append(ET.Element("ftp_images"))
            for u in (orig.split(sep) if sep in orig else [orig]):
                if u in map_:
...
def save_to_drive(b,f,map_):
    try:
        ...
        cred=json.loads(st.secrets.GOOGLE_DRIVE_CREDENTIALS_JSON)
...
def main():
    ...
    with tab1:
            ...
            if ft in ("xml","csv"):
                for enc in ("utf-8","iso-8859-2","windows-1250","utf-16-le","utf-16-be"):
                    try:
                        ...
                        fi=dict(content=content,raw_bytes=raw,type=ft,encoding=enc,name=fu.name)
                        break
                    except: pass
        ...
        if st.session_state.file_info:
            p=st.session_state.processing_params; fs=st.session_state.ftp_settings
            ...
            if st.button("Pobierz i prześlij"):
                ...
                for i in range(0,len(urls),10):
                    ...
                    save_state(
                        sid,
                        urls,
                        [d["original_url"] for d in all_dl],
                        {d["original_url"]: d["ftp_url"] for d in all_dl},
                        fi, p, all_fail
                    )
                prog.progress(1.0)
