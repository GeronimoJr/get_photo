...
class FTPManager:
        ...
        try:
            ...
            if self.settings["directory"] and self.settings["directory"] != "/":
                try:
                    ...
                    dirs = self.settings["directory"].strip("/").split("/")
    ...
    def upload_file(self, file_path, remote_filename=None):
        ...
        try:
            ...
            if self.settings.get("http_path") and self.settings["http_path"].strip():
                ...
                if self.settings["directory"] and self.settings["directory"] != "/":
                    if not self.settings["directory"].startswith("/"):
                        image_url += "/"
            ...
            self.connected = False
            if not self.connect():
                return {"success": False, "error": f"Utracono połączenie FTP i nie można ponownie połączyć: {str(e)}"}
            try:
                with open(file_path, 'rb') as file:
                    self.ftp.storbinary(f'STOR {remote_filename}', file)
                if self.settings.get("http_path") and self.settings["http_path"].strip():
                ...
                return {"success": False, "error": f"Nie udało się przesłać pliku nawet po ponownym połączeniu: {str(e2)}"}
        ...
        if file_type == "xml":
            if raw_bytes.startswith(codecs.BOM_UTF16_LE) or raw_bytes.startswith(codecs.BOM_UTF16_BE):
                try:
                    ...
                    return {"content": file_contents, "raw_bytes": raw_bytes, "type": file_type, "encoding": 'utf-16', "name": uploaded_file.name}, None
            ...
            if encoding_declared:
                ...
                try:
                    ...
                    return {"content": file_contents, "raw_bytes": raw_bytes, "type": file_type, "encoding": declared_encoding, "name": uploaded_file.name}, None
            ...
            for enc in encodings_to_try:
                try:
                    ...
                    return {"content": file_contents, "raw_bytes": raw_bytes, "type": file_type, "encoding": enc, "name": uploaded_file.name}, None
                except UnicodeDecodeError:
                    continue
        if file_type == "csv":
            try:
                ...
                return {"content": file_contents, "raw_bytes": raw_bytes, "type": file_type, "encoding": "auto-detected", "name": uploaded_file.name, "dataframe": df}, None
            except Exception:
                pass
        ...
        for future in concurrent.futures.as_completed(future_to_url):
            ...
            try:
                ...
                if result["status"] == "success":
                    ...
                    if debug_container:
                        debug_container.success(f"Pobrano i przesłano: {url} -> {result['ftp_url']}")
                else:
                    failed_urls.append({"url": url, "error": result.get("error", "Unknown error")})
                    if debug_container:
                        debug_container.warning(f"Błąd dla {url}: {result.get('error', 'Unknown error')}")
            except Exception as e:
...
def save_processing_state(session_id, urls, processed_urls, new_urls_map, file_info, processing_params):
    ...
    if "content" in file_info:
        state["file_content"] = file_info["content"]
        ...
        if filename.startswith("session_") and filename.endswith(".json"):
            try:
                ...
                sessions.append({
                    ...
                    "percentage": round(len(state["processed_urls"]) / state["total_urls"] * 100, 1) if state["total_urls"] > 0 else 0
                })
            except:
                pass
...
def resume_processing(state, temp_dir, ftp_settings, max_workers=5):
    ...
    if not remaining_urls:
        st.success("Nie ma URL-i do przetworzenia - wszystkie zostały już przetworzone.")
        return True
    ...
    for i in range(0, len(remaining_urls), batch_size):
        ...
        status_text.text(f"Przetwarzanie obrazów {len(processed_urls)+i+1}-{len(processed_urls)+min(i+batch_size, len(remaining_urls))} z {state['total_urls']}...")
        progress_value = (len(processed_urls) + i) / state["total_urls"]
        progress_bar.progress(progress_value)
        batch_result, batch_downloaded, batch_failed = process_images_in_parallel(
            batch_urls, temp_dir, ftp_settings, max_workers=max_workers, debug_container=debug_area
        )
        ...
        save_processing_state(
            session_id, state["remaining_urls"] + state["processed_urls"], processed_urls, new_urls_map, file_info, processing_params
        )
        ...
        if error:
            ...
            try:
                success, message = save_to_google_drive(
                    output_bytes, file_info, new_urls_map
                )
                if success:
    ...
    with tab1:
        ...
        with col1:
            st.session_state.ftp_settings["host"] = st.text_input(
                "Adres serwera FTP", value=st.session_state.ftp_settings["host"]
            )
            st.session_state.ftp_settings["port"] = st.number_input(
                "Port", value=st.session_state.ftp_settings["port"], min_value=1, max_value=65535
            )
            st.session_state.ftp_settings["directory"] = st.text_input(
                "Katalog docelowy", value=st.session_state.ftp_settings["directory"]
            )
        ...
        with col2:
            st.session_state.ftp_settings["username"] = st.text_input(
                "Nazwa użytkownika", value=st.session_state.ftp_settings["username"]
            )
            st.session_state.ftp_settings["password"] = st.text_input(
                "Hasło", type="password", value=st.session_state.ftp_settings["password"]
            )

        st.subheader("4. Pobierz zdjęcia i prześlij na FTP")
        if st.session_state.file_info:
            max_workers = st.slider(
                "Liczba równoległych procesów pobierania", min_value=1, max_value=10, value=3,
                    ...
                    if not st.session_state.ftp_settings["host"] or not st.session_state.ftp_settings["username"]:
                        ...
                        with tempfile.TemporaryDirectory() as tmpdirname:
                            ...
                            for i in range(0, len(urls), batch_size):
                                ...
                                batch_result, batch_downloaded, batch_failed = process_images_in_parallel(
                                    batch_urls, tmpdirname, st.session_state.ftp_settings, max_workers=max_workers, debug_container=debug_area
                                )
                                new_urls_map.update(batch_result)
                                processed_urls.extend(batch_urls)
                                save_processing_state(
                                    session_id, urls, processed_urls, new_urls_map, st.session_state.file_info, st.session_state.processing_params
                                )
                            ...
                            if new_urls_map:
                                ...
                                if error:
                                    ...
                                    try:
                                        success, message = save_to_google_drive(
                                            st.session_state.output_bytes, st.session_state.file_info, new_urls_map
                                        )
                                        if success:
                                            st.success(f"✅ {message}")
                                        else:
    ...
    with tab2:
        ...
        if not saved_sessions:
            ...
            selected_session_id = st.selectbox(
                ...
                options=[session["session_id"] for session in saved_sessions],
            ...
            max_workers_resume = st.slider(
                "Liczba równoległych procesów", min_value=1, max_value=10, value=3
            )
